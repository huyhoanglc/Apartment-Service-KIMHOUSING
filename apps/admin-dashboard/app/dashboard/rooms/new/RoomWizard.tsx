...
/**
 * Wizard tạo phòng mới - dùng chung cho trang standalone (/dashboard/rooms/new)
 * và side-panel "+ Thêm Phòng" ở trang Kho rổ hàng (xem RoomWizardSheet).
 * Khi nhúng trong side-panel, truyền onClose/onCreated thay vì để wizard tự điều hướng.
 */
export default function RoomWizard({
  ...
  onClose?: () => void;
  onCreated?: (room: { id: string; apartmentId: string }) => void;
}) {
  ...
  function goToInventory() {
    if (onClose) onClose();
    else router.push("/dashboard/apartments");
  }
  ...
  async function handleSaveRoom() {
    ...
    try {
      ...
      const created = { id: roomId, apartmentId: building.id };
